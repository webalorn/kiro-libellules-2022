...
def explore_sol(in_data, cur_time, realease_order, last_realeased, task_next, task_pool, assigned_times, predict_depth, machine_end, ope_end):
    ...
    for task_id in task_pool:
        for machine_id, ope_id in in_data['tasks']['using']:
    ...
    if predict_depth < MAX_DEPTH and len(ranked_pool):
        ...
        for task_to_do, (machine_id, ope_id) in ranked_pool:
            ...
            sub_score = explore_sol(in_data, cur_time, realease_order, last_realeased, task_next, new_pool, assigned_times, predict_depth+1, machine_end, ope_end)
        ...
        return explore_sol(in_data, cur_time+1, realease_order, last_realeased, task_next, task_pool, assigned_times, predict_depth, machine_end, ope_end)
    else:
...
def generate_with_exploration(in_data):
    ...
    assigned_to = [None] * n_data['nb_tasks']
    machine_end = [0] * in_data['nb_machines']
    ope_end = [0] * in_data['nb_operators']

    for _ in range(in_data['nb_tasks']):
        best_subtask_score, best_subtask, cur_time, last_realeased = (in_data, cur_time, realease_order, last_realeased, task_next, task_pool, assigned_times, 0, machine_end, ope_end)
